...
RUN CGO_ENABLED=0 GOOS=linux go build \
    ...
    -o /app/bin/sentiric-observability-service ./cmd/observability-service
...
COPY --from=builder /app/bin/sentiric-observability-service .
RUN chown appuser:appgroup ./sentiric-observability-service

# GÜVENLİK: Kullanıcıyı değiştir
USER appuser

ENTRYPOINT ["./sentiric-observability-service"]